...
class User_SQLighter(SQLighter):
    ...
    def get_users_nicknames(self):
        ...
        result = [user for user in result if user[0]
                  != None and user[1] != None]
        return result
    ...
    def check_if_admin(self, user_id):
        self.cursor.execute(
            f"SELECT isAdmin FROM subscriptions WHERE user_id = {user_id}")
    ...
    def account_exists(self, user_id):
        self.cursor.execute(
            f"SELECT hasAccount FROM subscriptions WHERE user_id = {user_id}")
        result = self.cursor.fetchone()
    ...
    def get_username(self, user_id):
        self.cursor.execute(
            f"SELECT username FROM subscriptions WHERE user_id = {user_id}")
        result = self.cursor.fetchone()
    ...
    def get_password(self, user_id):
        self.cursor.execute(
            f"SELECT password FROM subscriptions WHERE user_id = {user_id}")
        result = self.cursor.fetchone()
    ...
    def add_user(self, user_id):
        self.cursor.execute(
            f"INSERT INTO subscriptions (user_id) VALUES ({user_id})")
        self.commit()

    @SQLighter.check_connection
    def add_username(self, user_id, username):
        self.cursor.execute(
            f"UPDATE subscriptions SET username = '{username}' WHERE user_id = {user_id}")

    @SQLighter.check_connection
    def add_password(self, user_id, password):
        self.cursor.execute(
            f"UPDATE subscriptions SET password = {password} WHERE user_id = {user_id}")

    @SQLighter.check_connection
    def add_account(self, user_id):
        self.cursor.execute(
            f"UPDATE subscriptions SET hasAccount = {True} WHERE user_id = {user_id}")
        self.commit()
